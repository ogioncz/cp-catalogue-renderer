...
export class ItemGenerator implements WikitextGenerator {
	private template : WikitextTemplate;
	private findById = (list, id) => list.filter((item) => parseInt(item.paper_item_id) === id);
	...
	render(itemid) {
		var promise = new Promise(function(resolve, reject) {
			let promises = Utils.map(Utils.full_langs, (_, lang) => Utils.getJson('http://cdn.clubpenguin.com/play/' + lang + '/web_service/game_configs/paper_items.json'));
